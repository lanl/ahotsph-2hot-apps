...
static const body *subsample(
    int64_t *gnobj, int *nobj, body *btab, double subsample_fraction, int subsample_random_seed) {
    Stk outstk;
    StkInitEz(&outstk);
...
void output(const char *outnamebase,
    ...
    if (do_output && identsort_output) {
        pqsortsetup_order(&outputsort, output_btab, nobj, sizeof(outbody), 0.1, 1, Realloc_f);
    ...
    SDFwrite64(outname,
               ...
               "a_tvel",
               SDF_DOUBLE,
               cosmo->a_at_t(cosmo, tvel),
               "a_tacc",
               SDF_DOUBLE,
               cosmo->a_at_t(cosmo, tacc),
               "Omega0",
               SDF_DOUBLE,
               cosmo->Omega0,
               "Omega0_m",
               ...
               "growthfac",
               SDF_DOUBLE,
               cosmo->growthfac_at_z(cosmo, z),
               "growthfac_tvel",
               SDF_DOUBLE,
               cosmo->growthfac_at_t(cosmo, tvel),
               "growthfac_tacc",
               SDF_DOUBLE,
               cosmo->growthfac_at_t(cosmo, tacc),
               "velfac",
               SDF_DOUBLE,
               cosmo->velfac_at_z(cosmo, z),
               "tolerance",
               SDF_FLOAT,
               this_tol,
               "frac_tolerance",
