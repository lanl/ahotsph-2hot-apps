...
static void printActivity(CUpti_Activity *record) {
    switch (record->kind) {
        ...
        case CUPTI_ACTIVITY_KIND_CONTEXT: {
            ...
            printf(
                "CONTEXT %u, device %u, compute API %s\n",
                context->contextId,
                context->deviceId,
        ...
        case CUPTI_ACTIVITY_KIND_MEMCPY: {
            ...
            printf("MEMCPY %s %llu %llu stream %u corr %u r%u\n",
        ...
        case CUPTI_ACTIVITY_KIND_MEMSET: {
            ...
            printf(
                "MEMSET value=%u [ %llu - %llu ] device %u, context %u, stream %u, correlation "
                "%u/r%u\n",
                memset->value,
        ...
        case CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL: {
            ...
            printf("%s %s %llu %llu stream %u corr %u blocks %u threads %u\n",
                   kindString,
                   kernel->name,
...
bufferCompleted(CUcontext ctx, uint32_t streamId, uint8_t *buffer, size_t size, size_t validSize) {
    ...
    extern void recordActivity(CUpti_Activity * record);
...
void initTrace() {
    ...
    // printf("%s = %llu\n", "CUPTI_ACTIVITY_ATTR_DEVICE_BUFFER_POOL_LIMIT", (long long
    // unsigned)attrValue);
    attrValue *= 2;
    CUPTI_CALL(cuptiActivitySetAttribute(
        CUPTI_ACTIVITY_ATTR_DEVICE_BUFFER_POOL_LIMIT, &attrValueSize, &attrValue));
