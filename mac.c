...
void SetGravOffset(float *off, int n) {
    int i, j, k;
    int idx;
    int f = 2 * n + 1;
...
static struct { vsf mass, x, y, z; } Svec[SVECSZ];

#define MVECSZ (4 * 1873) /* This should be dynamically extensible */
static struct { vsf mass, x, y, z; } Mvec[MVECSZ];
...
static struct ucell_s { float halfsz, mass, x4, x2y2; } ucell[CHUBITS];
...
void SetupGrav(
    float newton_const, float e, int64_t gnobj, mac_s *m, float particle_mass, int smooth_type) {
    Nwarn = 0;
    GNewt = newton_const;
...
void WalkInitHSrc(hexacell *htab, int ncells) {
    Htab = htab;
...
void grav_mns_queue(
    int this_base, int this_m, const segment *this_seg, int this_seg_n, int this_source_n) {
    if (!mnsq.inuse) {
    ...
    Msgf((
        "queue %3d base %5d m %5d seg_n %5d -- ", mnsq.ss_len, mnsq.sink_base, mnsq.m, mnsq.seg_n));
    ...
    if (this_base < mnsq.sink_base + mnsq.m) {
        ...
        for (int i = 0; i < gap_m; i++) { mnsq.ss_index[mnsq.m + i] = mnsq.ss_len; }
        mnsq.m += gap_m;
        mnsq.ss_len++;
    }
    mnsq.m += this_m;
    ...
    if (base + this_m >= MNSQ_MAX_M)
        Error("mnsq.ss_index overflow\n");
    for (int i = 0; i < this_m; i++) { mnsq.ss_index[base + i] = mnsq.ss_len; }
    mnsq.seg_n += this_seg_n;
    mnsq.ss_len++;
    if (mnsq.seg_n >= MNSQ_MAX)
        Error("mnsq.seg_n overflow\n");
    if (mnsq.ss_len >= MNSQ_MAX / 16)
        Error("mnsq.ss_len overflow\n");
}

void grav_qns_queue(int this_base, int this_m, const Q_s *this_source, int this_source_n) {
    if (!qnsq.inuse) {
    ...
    Msgf(("queue base %5d m %5d source_len %5d -- ", qnsq.sink_base, qnsq.m, qnsq.source_len));
...
void grav_mns_flush(float mass, float e, int *ncut) {
...
void grav_qns_flush(void) {
    ...
    StartTimer(&GravQLTm);
    grav_qnss_CUDA(
        "pQ1_qnss", qnsq.sink_base, qnsq.m, qnsq.ss_seg, (float *)qnsq.source, qnsq.source_len, q);
...
void InheritSinkNlogN(const Sink *from, Sink *to, hcell *pp) {
    if (to == NULL) {
        ...
        StartTimer(&GravTm);
        if (bp - Btab >= Nobj - 1)
            grav_mns_flush(Btab[0].mass, e, &smooth_cnt);
#ifdef HEXA
        StartTimer(&GravHTm);
        nn = from->hcnt;
        while (nn % NSSE) {
        ...
        phid = 0.0;
        if (nn)
            Hinteract((float *)&Hvec[from->hcnt_done / NSSE],
                      (float *)&Hvec[nn / NSSE],
                      from->pos,
                      &mtot,
                      accd,
        ...
        phid = 0.0;
        if (nn)
            Qinteract((float *)&Qvec[from->qcnt_done / NSSE],
                      (float *)&Qvec[nn / NSSE],
                      from->pos,
                      &mtot,
                      accd,
        ...
        phid = 0.0;
        if (nn)
            Minteract((float *)&Mvec[from->mcnt_done / NSSE],
                      (float *)&Mvec[nn / NSSE],
                      from->pos,
                      &mtot,
                      accd,
        ...
        phid = 0.0;
        if (nn)
            MMinteract(Btab[0].pos,
                       sizeof(Btab[0]) / sizeof(float),
                       Btab[0].mass,
                       MMvec + from->mmcnt_done,
                       nn,
                       from->mmterms - from->mmterms_done,
                       from->pos,
                       &mtot,
                       accd,
                       &phid,
        ...
        if (nn)
            Sinteract((float *)&Svec[0],
                      ...
                      &e,
                      &smooth_cnt);
        if (from->scnt > BSMax.counter)
            BSMax.counter = from->scnt;
        AddCounter(&BSInt, from->scnt);
        if (smooth_cnt > BSMax.counter)
            BSMax.counter = smooth_cnt;
    ...
    if (Sub_Flags(pp)) {
        ...
        to->isbody = 0;
        if (cp->daughters <= 32 || 4.0001f * mac->r0 / mac->nx >= ucell[cp->level].halfsz)
            ccp = cp;
    } else {
        body *bp = pp->ptr;
        VV(to->pos, = bp->pos);
    ...
    if (from) {
        ...
        if (from->qqcnt >= QQVECSZ)
            Error("qqvec overflow\n");
        if (!from->processed && mac->p2cut && MxN->hblock && from->daughters >= MxN->min_qsink
...
#endif
        if (from->mmcnt >= MMVECSZ)
            Error("mmvec overflow\n");
        ...
        if (!from->processed)
            ((Sink *)from)->processed = 1;
        to->scnt = from->scnt;
        if (to->scnt >= NSSE * SVECSZ)
            Error("svec overflow\n");
...
void mxn_mono(Sink *s, const hcell *pp) {
    ...
    if (!first || !last)
        return;
    last++;
    if (first < Btab || last > first + Nobj)
        Error("first/last out of range\n");
    StartTimer(&GravTm);
    StartTimer(&GravMFTm);
    ...
    if (MxN->do_pM && first - Btab >= mnsq.sink_base + mnsq.m) { /* Don't do more than one level */
        ...
        if (mnsq.seg_n >= 32768)
            grav_mns_flush(Btab[0].mass, s->smooth_len, &s->smooth_cnt);
    } else {
        float mtot = 0.0;
...
void mxn_quad2(Sink *s, const hcell *pp) {
    body *first = FirstBody(pp);
    body *last = LastBody(pp);
    ...
    if (MxN->do_pQ) {
        if (MxN->do_pQL && last - first < 256
            && first - Btab >= qnsq.sink_base + qnsq.m) { /* Don't do more than one level */
            ...
            if (q >= 0 && qinuse < 24) {
                ...
                grav_mn_CUDA("pQ",
                             ...
                             (float *)&Qvec[s->qcnt_done / NSSE],
                             source_n,
                             sizeof(Qvec[0]) / (NSSE * sizeof(float)),
                             s->smooth_len,
                             &s->smooth_cnt,
                             q);
                StopTimer(&GravQFTm);
                ...
                for (body *p = first; p < last; p++) {
                    float mtot = 0.0;
                    Qinteract((float *)&Qvec[s->qcnt_done / NSSE],
                              (float *)&Qvec[s->qcnt / NSSE],
                              p->pos,
                              &mtot,
                              p->acc,
                              &p->phi,
...
void mxn_quad(Sink *s, const hcell *pp) {
    body *p;
    body *first = FirstBody(pp);
    body *last = LastBody(pp);
    ...
    if (n1 - n0 > MxN->hblock)
        ...
        Error("mxn_quad called with n == 0\n");
    else
        m_block = MxN->hblock / (n1 - n0);
    block = m_block;
...
#else
            pQinteract(
                &p->mass, p->acc, block, sizeof(body) / sizeof(float), (float *)&Qvec[n0], n1 - n0);
#endif
        } else {
            float mtot = 0.0f;
            float e = 0.0f;
...
void mxn_qquad(Sink *s, const hcell *pp) {
    body *first = FirstBody(pp);
    body *last = LastBody(pp);
...
void mxn_hexa(Sink *s, const hcell *pp) {
    body *p;
    body *first = FirstBody(pp);
    body *last = LastBody(pp);
    int i, n0, n1, m_block, block;
    ...
    if (n1 - n0 > MxN->hblock)
        ...
        Error("mxn_hexa called with n == 0\n");
    else
        m_block = MxN->hblock / (n1 - n0);
    block = m_block;
    for (p = first; p < last; p += m_block) {
        ...
        if (MxN->do_pH) {
            ...
            if (q >= 0) {
                grav_mn_CUDA("pH",
                             ...
                             (float *)&Hvec[n0],
                             (n1 - n0) * NSSE,
                             sizeof(Hvec[0]) / (NSSE * sizeof(float)),
                             s->smooth_len,
                             &s->smooth_cnt,
                             q);
            } else {
...
#else
            pHinteract(
                &p->mass, p->acc, block, sizeof(body) / sizeof(float), (float *)&Hvec[n0], n1 - n0);
#endif
        } else {
            float mtot = 0.0f;
            float e = 0.0f;
...
void RcritMAC(Sink *sink, const hcell **source_vec, int *flags, int *result, int n) {
#if 0
...
void DLRcritMACsb(
    Sink *sink, const hcell **source_vec, int *restrict flags_vec, int *restrict result, int n) {
    float dr2;
    Vxd(float r);
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sf) {
            ...
            if (!bs && cp->level == sink->clevel && ddot(r, sink->cen) < sink->cr2) {
                ...
                DebugWatchKey("b %12g %12g Near %ld %s\n",
                              0.0,
                              sqrt(ddot(r, sink->cen)),
                              (long int)cp->daughters,
                              PrintKey(source_vec[i]->key));
            }
            if (isquad && dr2 > Square(qcp->rcrit_q + bmax)) {
                if (0 && !bs && MxN->do_pQL && offset_index(flags_vec[i]) == Nimage >> 1
                    && !(source_vec[i]->type & NONLOCAL)) {
                ...
                result[i] = (mac->dlfac * sink->bmax > smallest_rcrit
                             && sink->daughters > mac->leaf_max_n)
                                ? MAC_SPLIT_SINK
                                : MAC_SPLIT_SRC;
            }
            DebugWatchKey("%10g %10g %10ld %s\n",
                          ...
                          smallest_rcrit,
                          (long int)cp->daughters,
                          (result[i] == MAC_ACCEPT)       ? "Accept"
                          : (result[i] == MAC_SPLIT_SINK) ? "Split Sink"
                                                          : "Split Source");
        } else {
            /* body-body */
        ...
        if (result[i] == MAC_SPLIT_SRC && !bs) {
            if (cp->level >= sink->clevel) {
                ...
                DebugWatchKey("  %12g %12g Cvec %ld %s\n",
                       ...
                       && cp->level > 4) { /* assumes empty cells near root are from expand_root */
        ...
        if (result[i] != MAC_ACCEPT && cp->daughters <= mac->leaf_max_n
            && !(source_vec[i]->type & (SHARED | NONLOCAL))) {
    ...
    if (sink->scnt / NSSE >= SVECSZ)
        Error("svec overflow\n");
    if (sink->mcnt / NSSE >= MVECSZ)
        Error("mvec overflow\n");
    if (sink->mmcnt >= MMVECSZ)
        Error("mmvec overflow\n");
    if (sink->qcnt / NSSE >= QVECSZ)
        Error("qvec overflow\n");
    if (sink->qqcnt >= QQVECSZ)
        Error("qqvec overflow\n");
    if (sink->hcnt / NSSE >= HVECSZ)
        Error("hvec overflow\n");
}
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sf) {
            ...
            if (dr2 > Square(cp->rcrit + cp->bmax)) {
                ...
                result[i]
                    = mac->dlfac * sink->bmax > smallest_rcrit ? MAC_SPLIT_SINK : MAC_SPLIT_SRC;
            }
        } else {
            /* body-body */
            appendMvec(cp, cp->mass);
