...
/* 217 flops, 100 bytes */
void do_gravdh_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravdh_amd6100_avx8(const vsf *f,
                            const vsf *fend,
                            const float *pos0,
                            float *mass0,
                            float *acc,
...
/* 66 flops, 40 bytes */
void do_gravdq_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravdqq_avx8(const float *q,
                     const int stride,
                     const int *ii,
                     const int n,
                     const float *pos0,
...
void do_gravh_avx8(const vsf *f,
                   const vsf *fend,
                   const float *pos0,
                   float *mass0,
                   float *acc,
...
void do_gravh_amd6100_avx8(const vsf *f,
                           const vsf *fend,
                           const float *pos0,
                           float *mass0,
                           float *acc,
...
void do_gravq_avx8(const vsf *f,
                   const vsf *fend,
                   const float *pos0,
                   float *mass0,
                   float *acc,
...
void do_grav_sK1_avx8(const vsf *f,
                      const vsf *fend,
                      const float *pos0,
                      float *mass0,
                      float *acc,
...
void do_gravmm_sK1_avx8(const float *xyz,
                        const int stride,
                        const float pmass,
                        const segment *mm,
                        const int mm_n,
...
void do_gravsS_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravsF1_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
void do_gravsF2_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
/* Dehnen K1 Compensating Kernel */
void do_gravsK1_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
void do_gravsK2_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
void do_gravsK3_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
void do_gravsU_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravsCP_avx8(const vsf *f,
                     const vsf *fend,
                     const float *pos0,
                     float *mass0,
                     float *acc,
...
void do_grav11bits_avx8(const vsf *f,
                        const vsf *fend,
                        const float *pos0,
                        float *mass0,
                        float *acc,
...
void do_gravph_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravph_amd6100_avx8(const vsf *f,
                            const vsf *fend,
                            const float *pos0,
                            float *mass0,
                            float *acc,
...
void do_gravpq_avx8(const vsf *f,
                    const vsf *fend,
                    const float *pos0,
                    float *mass0,
                    float *acc,
...
void do_gravp_avx8(const vsf *f,
                   const vsf *fend,
                   const float *pos0,
                   float *mass0,
                   float *acc,
...
void do_gravp11bits_avx8(const vsf *f,
                         const vsf *fend,
                         const float *pos0,
                         float *mass0,
                         float *acc,
