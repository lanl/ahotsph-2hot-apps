...
__global__ void pH(
    const float *p, float *ret, const int n, const int stride, const float *f, const int source_n) {
    int i, ii;
    ...
    float xxx[VECWIDTH], xxy[VECWIDTH], xyy[VECWIDTH], yyy[VECWIDTH], xxz[VECWIDTH], xyz[VECWIDTH],
        yyz[VECWIDTH];
...
__global__ void pQ1_mnss(const float *__restrict__ sink,
                         const int sink_m,
                         const segment *__restrict__ seg,
...
__global__ void pM_sK1(const float *p,
                       float *ret,
                       const int n,
                       const int stride,
                       const float *f,
    ...
    for (i = 0; i < source_n; i++) {
        ...
        for (int j = 0; j < VECWIDTH; j++) {
            if (r2[j] <= eps2) {
                ...
                t[j] = (((45.0f / 32.0f) * u2[j] + (-3.0f / 8.0f)) * u2[j] + (1.0f / 2.0f)) * u2[j]
                       - 1.0f;
                t[j] *= eps_inv;
...
__global__ void pQQ1(const float *__restrict__ sink,
                     const int sink_m,
...
__global__ void pMM_sK1(const float *__restrict__ sink,
                        const int sink_m,
                        const segment *__restrict__ seg,
                        const int seg_len,
        ...
        for (int seg_j = 0; seg_j < seg[seg_i].length; seg_j++) {
            ...
            for (int j = 0; j < VECWIDTH; j++) {
                ...
                if (r2[j] > eps2) {
                    ...
                    u2[j] = r2[j] * eps_inv2 - 1.0f;
                    t[j] = (((45.0f / 32.0f) * u2[j] + (-3.0f / 8.0f)) * u2[j] + (1.0f / 2.0f))
                               * u2[j]
                           - 1.0f;
                    t[j] *= mmass * eps_inv;
                    acc[j].Phi += t[j];
...
__global__ void pMM_mnss_sK1(const float *__restrict__ sink,
                             const int sink_m,
                             const uint16_t *__restrict__ ss_index,
                             const segment *__restrict__ ss_seg,
                             const int ss_len,
                             const segment *__restrict__ s_seg,
                             const int s_seg_len,
                             const float *__restrict__ source,
                             const int *s_source_n,
                             const float mmass,
                             const float eps_inv,
                             float *accp,
                             int *ncut) {
...
__global__ void pMM1_mnss_sK1(const float *__restrict__ sink,
                              const int sink_m,
                              const uint16_t *__restrict__ ss_index,
                              const segment *__restrict__ ss_seg,
                              const int ss_len,
    ...
    const int seg_len = ss_seg[sindex].length;
    const int source_n = s_source_n[sindex];
    // if (debug) printf("%d %d %d %d %d\n", index, sindex, ss_len, ss_seg[sindex].base,
    // ss_seg[sindex].length);
    ...
    for (int seg_i = 0; seg_i < seg_len; seg_i++) {
        ...
        for (int seg_j = 0; seg_j < seg[seg_i].length; seg_j++) {
            ...
            if (r2 > eps2) {
                ...
                float t
                    = (((45.0f / 32.0f) * u2 + (-3.0f / 8.0f)) * u2 + (1.0f / 2.0f)) * u2 - 1.0f;
                t *= mmass * eps_inv;
                acc.Phi += t;
...
static int check_cudaq(int *inuse) {
    int i;
    cudaError_t err;
    int ninuse = 0;
...
extern "C" void WalkInitSrcCUDA(float *qtab, int stride, int64_t ncells) {
...
extern "C" void WalkTerminateSrcCUDA(void) {
    cudaError_t err = cudaFree(qdev);
    if (err != cudaSuccess)
        Error("cudaFree failed, %d %s\n", err, cudaGetErrorString(err));
}

extern "C" void WalkInitSinkCUDA(float *btab, int stride, int64_t nobj) {
...
extern "C" void WalkTerminateSinkCUDA(float *btab, int stride, int64_t nobj) {
    ...
    for (int64_t i = 0; i < nobj; i++) {
        ...
        // Msgf(("b %5ld %12g %12g %12g\n", i, btab[i*stride+7], btab[i*stride+8],
        // btab[i*stride+9]));
    }
...
extern "C" int vecwidthCUDA(void) { return VECWIDTH; }

extern "C" int qallocCUDA(int *inuse) { return check_cudaq(inuse); }

extern "C" void grav_mn_CUDA(const char *routine,
                             const float *p,
                             float *accp,
                             const int m,
                             const int stride,
    ...
    err = cudaMemcpyAsync(dev, cudaq[q].host, total_size, cudaMemcpyHostToDevice, cudaq[q].stream);
    if (err != cudaSuccess)
        Error("cudaMemcpy failed, %d %s\n", err, cudaGetErrorString(err));
...
extern "C" void grav_mns_CUDA(const char *routine,
                              const int base,
                              const int m,
                              const segment *seg,
                              const int seg_n,
    ...
    err = cudaMemcpyAsync(
        devseg, cudaq[q].host, total_size, cudaMemcpyHostToDevice, cudaq[q].stream);
    if (err != cudaSuccess)
        Error("cudaMemcpy failed, %d %s\n", err, cudaGetErrorString(err));
    AddCounter(&PCISend, total_size);
...
extern "C" void grav_qns_CUDA(const char *routine,
                              const int base,
                              const int m,
                              const int *source_list,
                              const int source_n,
                              int q) {
    cudaError_t err;
    ...
    if (strcmp(routine, "pQQ1") == 0)
        pQQ1<<<blocks, threads, 0, stream>>>(sink, m, source, source_n, qdev, accp);
    else
        Error("routine %s not found\n", routine);
...
extern "C" void grav_mnss_CUDA(const char *routine,
                               const int sink_base,
                               const int m,
                               const uint16_t *ss_index,
                               const segment *ss_seg,
                               const int ss_len,
                               const segment *seg,
                               const int seg_len,
    ...
    if (strcmp(routine, "pMM_mnss_sK1") == 0)
        pMM1_mnss_sK1<<<blocks, threads, 0, stream>>>(sink,
                                                      m,
                                                      ss_index_dev,
                                                      ss_seg_dev,
                                                      ss_len,
...
extern "C" void grav_qnss_CUDA(const char *routine,
                               const int sink_base,
                               const int m,
                               const segment *ss_seg,
    ...
    if (strcmp(routine, "pQ1_qnss") == 0)
        pQ1_mnss<<<blocks, threads, 0, stream>>>(sink, m, ss_seg_dev, source_dev, accp);
    else
        Error("routine %s not found\n", routine);
...
void recordActivity(CUpti_Activity *record) {
